...
import { Ordem, OrdemData } from "../../../@core/data/ordem";
...
export class OrdemComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(
    private classeService: ClasseData,
    private service: OrdemData,
  ...
  search = (text$: Observable<string>): any =>
    text$.pipe(
      ...
      map((term) =>
        term.length == 0
          ? []
          : this.classes.filter((v) => v.nome.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10),
      ),
    );
  ...
  public async submitHandler(): Promise<any> {
    this.submitted = true;
    if (typeof this.form.value.classe !== "object" || this.form.value.classe == "") {
