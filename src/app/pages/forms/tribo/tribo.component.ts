import { Supertribo, SupertriboData, Tribo, TriboData } from "./../../../@core/data/tribo";
...
import { FamiliaData, SubfamiliaData } from "../../../@core/data/familia";

@Component({
  selector: "ngx-tribo",
  templateUrl: "./tribo.component.html",
  styleUrls: ["./tribo.component.scss"],
})
export class TriboComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(
    ...
    private subfamiliaService: SubfamiliaData,
    private supertriboService: SupertriboData,
    private service: TriboData,
    ...
    this.supertriboService
      .getListData()
      .pipe(takeWhile((): boolean => this.alive))
      .subscribe((data): void => {
        this.supertribos = data;
      });
  ...
  searchSub = (text$: Observable<string>): any =>
    text$.pipe(
      ...
      map((term) =>
        term.length == 0
          ? []
          : this.subfamilias.filter((v) => v.nome.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10),
      ),
    );

  searchSupertribo = (text$: Observable<string>): any =>
    text$.pipe(
      debounceTime(200),
      distinctUntilChanged(),
      map((term) =>
        term.length == 0
          ? []
          : this.supertribos.filter((v) => v.nome.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10),
      ),
    );
  ...
  public async submitHandler(): Promise<any> {
    ...
    if (this.form.value.subfamilia == "") {
      this.form.value.subfamilia = null;
    }
    if (this.form.value.supertribo == "") {
      this.form.value.supertribo = null;
    }
  ...
  public delete(object): void {
    Swal.fire({
      ...
      if (result.value) {
        this.service.deleteData(Number(object.id_tribo)).subscribe(
          (): void => {
